<?php
	...
	$pdo_statement = $DB_con->prepare("SELECT * FROM `attendance` INNER JOIN `user` ON `attendance`.`rfid` = `user`.`rfid` WHERE (`attendance`.`adate` BETWEEN :from AND :to) AND `attendance`.`istardy` = 1 AND `user`.`position` ='Student' AND `attendance`.`location` LIKE '%In%' ");
