                                ...
                                <div class="row ">
                                    <div class="col-lg-12">
                                        <table id="userlist" class="display table table-stripped table-fluid"  >
                                            ...
                                            <tbody>
                                            <?php
                                            $pdo_statement = $DB_con->prepare("SELECT * FROM users24 INNER JOIN s_recommendations ON users24.username = s_recommendations.user_id  WHERE users24.position = :position AND users24.status = :status AND s_recommendations.star = 1 ORDER BY users24.id DESC");
                                                    ...
                                                    <form>
                                                        ...
                                                        <td>
                                                            ...
                                                            foreach($recs as $reco) {
                                                                if (!empty($reco["esl"])) {
                                                                    echo "<span class='text-success'>&check; ESL Required</span><br>";
                                                                }

                                                                if (!empty($reco["star"])) {
                                                                    echo "<span class='text-success'>&check; STAR Required</span><br>";
                                                                }

                                                                if (!empty($reco["completion"])) {
                                                                    echo "<span class='text-success'>&check; Completion</span>";
                                                                }
                                                            }
                                                            ?>
                                                        </td>
