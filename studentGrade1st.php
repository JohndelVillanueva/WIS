                        ...
                        <tbody>
                            <?php foreach ($getAllSubjects as $studentSubject) : ?>
                                <tr>
                                    ...
                                    $getAllActivityType = $DB_con->prepare("SELECT SUM(`score`) as totalScore, SUM(`maxscore`) as totalMaxscore, `acttype` FROM s_scores 
                                    WHERE subjcode = :subjcode AND qtr = :qtr AND sid = :sid
                                    GROUP BY acttype, subjcode");
                                    $getAllActivityType->execute([":subjcode" => $studentSubject->code, ":qtr" => $_GET['qtr'], ":sid" => $_SESSION['username']]);
                                    $getActivityType = $getAllActivityType->fetchAll(PDO::FETCH_OBJ);

                                    $totalScore = 0;
                                    $totalMaxscore = 0;

                                    foreach ($getActivityType as $activityType) {
                                    $totalScore += $activityType->totalScore;
                                    $totalMaxscore += $activityType->totalMaxscore;
                                    }

                                    $accttype = array_filter($getActivityType, function($data) {
                                    return $data->acttype === 3;
                                    });

                                    $accttype2 = $accttype[0] ?? []; // Make sure $accttype2 is an array or object
                                    var_dump($accttype2);

                                    if ($totalMaxscore != 0) {
                                        $activityTotal = ($totalScore / $totalMaxscore) * 100;
                                        $getTotalGrade = round($activityTotal, 2);
                                        $getFinalGrade = round(round($getTotalGrade, 2) * $studentSubject->percentww, 2);
                                        var_dump($getFinalGrade);

                                    // print_r(["test", $writtenGrades]);
