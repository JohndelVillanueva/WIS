						...
						<div class="row" style="margin-top:-30px;">
							...
							if(!empty($result)) {
								foreach($result as $row) {
                                    ...
                                    $insert->execute(array( ':rfid'=>$_POST['rfid'],':vname'=>ucwords(strtolower($_POST['guestname'])),':remarks'=>$_POST['guestcomment']));

                                    $update = $DB_con->prepare("UPDATE `user` SET `fname` = :fname WHERE `user`.`rfid` = :rfid");
                                    $update->execute(array( ':fname'=>ucwords(strtolower($_POST['guestname'])),':rfid'=>$_POST['rfid']));
                                }
                            }
							?>
